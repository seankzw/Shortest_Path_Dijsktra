...
def findStartBus(data, userLoc):
    # try:
	shortestDistance = 1000
	minLat = 10000
	minLng = 100000		
	for i in data:
		if distance(data[i]["coordinates"][0], userLoc[0], data[i]["coordinates"][1], userLoc[1])< shortestDistance:
			minLat = data[i]["coordinates"][0]
			minLng = data[i]["coordinates"][1]
			shortestDistance = distance(data[i]["coordinates"][0], userLoc[0], data[i]["coordinates"][1], userLoc[1])
			index = i
